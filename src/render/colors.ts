...
export function getContextColor(percent: number): string {
  if (percent >= 85) return FG_RED;
  if (percent >= 70) return FG_YELLOW;
  return FG_GREEN;
}
...
export function alertColor(level: 'warning' | 'critical' | 'danger'): string {
  switch (level) {
    case 'danger':
...
export function getCostColor(cost: number): string {
  if (cost >= 1) return FG_RED;
  if (cost >= 0.1) return FG_YELLOW;
  return FG_GREEN;
}
