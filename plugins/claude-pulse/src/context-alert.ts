...
export function checkContextAlert(
  ...
  let level: 'warning' | 'critical' | 'danger' | null = null;
 ...
 */
function getAlertIcon(level: 'warning' | 'critical' | 'danger'): string {
  switch (level) {
    case 'danger':
      return '🚨';
    case 'critical':
 ...
 */
export function shouldPlaySound(alert: ContextAlert, config: PulseConfig): boolean {
  return (
    config.alerts.soundEnabled &&
    alert.triggered &&
 ...
 */
export function getTerminalBell(): string {
