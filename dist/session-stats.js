 ...
 */
export function calculateStats(transcript) {
    ...
    const completedWithDuration = tools.filter(t => t.status === 'completed' && t.endTime && t.startTime);
    ...
    const sortedTools = new Map([...toolCounts.entries()].sort((a, b) => b[1] - a[1]));
    // Agent statistics
 ...
 */
export function getTopTools(stats, n = 3) {
    const result = [];
    let count = 0;
 ...
 */
export function getRunningToolsInfo(tools) {
    const runningTools = tools.filter(t => t.status === 'running');
    const count = runningTools.length;
    if (count === 0) {
        return { count: 0, oldest: null };
    }
    const oldest = runningTools.reduce((prev, curr) => curr.startTime < prev.startTime ? curr : prev);
